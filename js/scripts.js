...
function updateWeekday() {
    ...
    weekdayElement.style.background = `linear-gradient(45deg, ${weekdayColors[weekday]}, #000000)`;
    weekdayElement.style.color = "#ffffff"
}
...
function updateScheduleCountdown() {
    ...
    const cycleLength = sessionLength + breakLength; //110 minutes in milliseconds or 6,600,000 ms
    ...
    document.getElementById("testingtext").textContent = "cyclesDone = " + Math.floor(timeSinceStart/cycleLength) + " // timeSinceStart = " + Math.floor(timeSinceStart/60000) + " // timeInCurrentCycle = " + Math.floor(timeInCurrentCycle/60000) + " // timeRemaining = " + Math.floor(timeRemaining/60000) + " // timeRemaining2 = " + Math.floor(timeRemaining2/60000);

    // Calculate sessionProgress here, where timeInCurrentCycle is defined
    // 110 minutes in milliseconds = 110 * 60 * 1000
    // sessionProgress should be percentage of current cycle completed (0-100)
    let setProgress = ( amount ) => {
        amount = amount || 0;
        document.getElementById( 'progressBar2' ).style.backgroundImage = 'linear-gradient(90deg, #ffffffff ' + amount + '%, rgba(0, 0, 0, 1) ' + amount + '%)';
        document.getElementById( "schedule-countdown" ).style.backgroundImage = 'linear-gradient(90deg, #000000ff ' + amount + '%, rgba(255, 255, 255, 1) ' + amount + '%)';
    }

    let sessionProgress = (timeInCurrentCycle / (cycleLength-1200000)) * 100 ;
    // You can use sessionProgress here as needed, e.g.:
...
// sessionProgress is now calculated inside updateScheduleCountdown
